...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const adjustMenu = (init) => {
        const getAllWidth = (ele) => {
            return Array.from(ele).reduce(
                (width, i) => width + i.offsetWidth,
                0
            );
        };
        ...
        const hideMenuIndex =
            ...
            headerContentWidth > $nav.offsetWidth - 120;
        ...
        const runJustifiedGallery = (item, arr) => {
            ...
            if (!item.classList.contains("lazyload") || arr.length < limit) {
                ...
                if (
                    ...
                    item.nextElementSibling.addEventListener(
                        "click",
                        clickBtnFn
                    );
                }
        ...
        function percentageScrollFn(currentTop) {
            ...
            if (waterfallDom) {
                const waterfallResult =
                    currentTop % document.documentElement.clientHeight; // 捲去一個視口
                if (
    ...
    const handleThemeChange = (mode) => {
        ...
        const menuDarkmodeText = $rightMenu.querySelector(
            ".menu-darkmode-text"
        );
        if (mode === "light") {
            menuDarkmodeText.textContent = "Light / Dark";
        } else {
            menuDarkmodeText.textContent = "Light / Dark";
        }
    ...
    const rightSideFn = {
        ...
        "hide-aside-btn": () => {
            ...
            const saveStatus = $htmlDom.contains("hide-aside")
                ? "show"
                : "hide";
        ...
        function handleMouseOver(e) {
            if (e.target.tagName == "IMG" && flag) {
                ...
                owo_time = setTimeout(() => {
                    ...
                    let left =
                        e.x - e.offsetX - (width - e.target.clientWidth) / 2; // 盒子與屏幕左邊距離
    ...
    const listenNavMusicPause = function () {
        const timer = setInterval(() => {
            if (
                ...
                clearInterval(timer);
                let msgPlay =
                    '<i class="anzhiyufont anzhiyu-icon-play"></i><span>Play music</span>';
                let msgPause =
                    '<i class="anzhiyufont anzhiyu-icon-pause"></i><span>Pause music</span>';
                navMusicEl
